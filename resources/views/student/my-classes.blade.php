                                        ...
                                        @foreach($classrooms as $classroom)
                                            <tr>
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium flex flex-row justify-end">
                                                    <div>
                                                        <form action="{{route('classrooms.unsubscribe', $classroom->id)}}" method="post">
                                                            @csrf
                                                            <x-jet-button>
                                                                <i class="fa fa-eye"></i>
                                                                {{ __('Unsubscribe') }}
...
{{--                                                    <x-jet-link-button :href="route('classrooms.show', $classroom->id)">--}}
{{--                                                        <i class="fa fa-eye"></i>--}}
{{--                                                        {{ __('View') }}--}}
{{--                                                    </x-jet-link-button>--}}

                                                    @unless($classroom->status == 0)
                                                        <x-jet-link-button :href="route('classrooms.join', $classroom->room_id)" class="text-xs">
                                                            <i class="fa fa-eye"></i>
                                                            {{ __('Join') }}
                                                        </x-jet-link-button>
                                                    @endunless
